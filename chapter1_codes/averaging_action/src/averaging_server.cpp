...
static void stat(const std::vector<double> &arr, double &mean,
                 double &std_dev) {
  int N = arr.size();
  double sum = 0;
  for (int i = 0; i < N; ++i)
...
protected:
  ...
  actionlib::SimpleActionServer<averaging_action::AveragingAction>
      action_server_;
...
public:
  AveragingAction(std::string name)
      : action_server_(node_handle_, name,
                       boost::bind(&AveragingAction::executeCB, this, _1),
                       false),
        name_(name), engine_(seed_gen_()) {
    // set limit
    std::uniform_real_distribution<>::param_type param(0.0, 1.0);
    dist_.param(param);
    // start server
    action_server_.registerPreemptCallback(
        boost::bind(&AveragingAction::preemptCallback, this));
  ...
  void executeCB(const averaging_action::AveragingGoalConstPtr &goal) {
    ...
    if (data_count_ >= sample_number_goal) {
      ROS_INFO("already generated %d numbers, returning to zero",
               sample_number_goal);
      clearAll();
