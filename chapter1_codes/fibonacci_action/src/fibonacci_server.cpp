...
FibonacciAction::FibonacciAction(const std::string name):
    action_server_(node_handle_, name, boost::bind(&FibonacciAction::executeCallback, this, _1), false),
    name_(name)
{
...
void FibonacciAction::executeCallback(const fibonacci_action::FibonacciGoalConstPtr &goal){
    ...
    ROS_INFO("%s: Executing. Creating Fibonacci sequence of order %d with seeds %d, %d",
