...
protected:
  ...
  actionlib::SimpleActionServer<fibonacci_action::FibonacciAction>
      action_server_;
...
FibonacciAction::FibonacciAction(const std::string name)
    : action_server_(node_handle_, name,
                     boost::bind(&FibonacciAction::executeCallback, this, _1),
                     false),
      name_(name) {
  action_server_.start();
}

void FibonacciAction::executeCallback(
    const fibonacci_action::FibonacciGoalConstPtr &goal) {
  ros::Rate rate(1);
  bool success = true;
  ...
  ROS_INFO("%s: Executing. Creating Fibonacci sequence of order %d with seeds "
           "%d, %d",
           name_.c_str(), goal->order, feedback_.sequence[0],
  ...
  for (int i = 1; i <= goal->order; i++) {
    ...
    feedback_.sequence.push_back(feedback_.sequence[i - 1] +
