...
public:
  demo_actionAction(std::string name)
      : action_server_(
            node_handle_, name,
            boost::bind(&demo_actionAction::executeCallback, this, _1), false),
        action_name_(name), goal_(0), progress_(0) {
    action_server_.registerPreemptCallback(
        boost::bind(&demo_actionAction::preemptCallback, this));
  ...
  void
  executeCallback(const mastering_ros_demo_pkg::demo_actionGoalConstPtr &goal) {
    ...
    ros::Rate rate(5);
    ROS_INFO("%s is progressing the goal %d", action_name_.c_str(),
             goal->goal_deg);
    ...
    while (progress_ <= goal->goal_deg) {
      ...
      if (progress_ >= goal->goal_deg) {
        ROS_INFO("%s succeeded in getting to goal %d", action_name_.c_str(),
                 goal->goal_deg);
        result_.final_deg = progress_;
        action_server_.setSucceeded(result_);
      }
      ...
      // main operation
        ...
        ROS_INFO("setting to goal %d / %d", feedback_.current_deg,
                 goal->goal_deg);
