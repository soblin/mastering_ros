...
void callback(const mastering_ros_demo_pkg::data_array::ConstPtr &msg) {
  ...
  for (int i = 0; i < len; i++) {
    ROS_INFO("upperLeft:%d, lowerRight:%d, color:%s, cameraID:%s",
             msg->img_data[i].upperLeft, msg->img_data[i].lowerRight,
             msg->img_data[i].color.c_str(), msg->img_data[i].cameraID.c_str());
  }
  ROS_INFO("That's all.");
}
...
int main(int argc, char **argv) {
  ...
  ros::Subscriber array_msg_susbcriber_node =
      node_obj.subscribe<mastering_ros_demo_pkg::data_array>(
          "/array_msg_publisher_topic", 50, callback);
  ros::spin();
  return 0;
}
