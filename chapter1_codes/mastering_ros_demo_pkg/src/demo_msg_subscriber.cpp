...
void callback(const mastering_ros_demo_pkg::full_message::ConstPtr &msg) {
  ...
  for (int i = 0; i < msg->messages.size(); i++) {
    ...
    ROS_INFO("{greeting1:%s, number:%d, unumber:%d, greeting2:%s, fnumber:%f, "
             "TrueOrFalse:%s}\n",
             unit_msg.greeting1.c_str(), unit_msg.number, unit_msg.unumber,
             unit_msg.greeting2.c_str(), unit_msg.fnumber,
             (unit_msg.TrueOrFalse ? "true" : "false"));
  }
  ROS_INFO("\n\n");
}
...
int main(int argc, char **argv) {
  ...
  ros::Subscriber demo_msg_subscriber_node =
      node_obj.subscribe<mastering_ros_demo_pkg::full_message>(
          "/demo_msg_publisher_topic", 50, callback);
  ros::spin();
  return 0;
}
