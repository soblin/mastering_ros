...
#include <mastering_ros_demo_pkg/demo_serviceResponse.h>
#include <mastering_ros_demo_pkg/demo_serviceRequest.h>
...
//ConstPtr使いたかったけどエラー出る
bool callback(mastering_ros_demo_pkg::demo_service::Request  &req,
              mastering_ros_demo_pkg::demo_service::Response &res){
    std::stringstream ss;
    int number = req.number;
    if(nabeatsu(number)){
        ss << std::to_string(number) + "!!!!!!!!!!!";
    }
    else ss << std::to_string(number) + " is a trivial number";
